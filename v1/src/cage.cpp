...
void Cage::AddAnimal(char animal, int i) {  
  ...
  cout << h[pos_id].GetCellRow() << " " << h[pos_id].GetCellCol() << endl; 
}
      ...
      switch(j) {
        case 1:
          for (int k = 0; k <= neff; k++) {
            int y = a[i]->GetY()-1;
            if (a[i]->GetX() != a[k]->GetX() && y != a[k]->GetY() && InsideCage(a[i]->GetX(),y)) {
              a[i]->Move(1);
              found = true;
            }
          }
        ...
        case 2:
          for (int k = 0; k <= neff; k++) {
            int x = a[i]->GetX()+1;
            if (x != a[k]->GetX() && a[i]->GetY() != a[k]->GetY() && InsideCage(x,a[i]->GetY())) {
              a[i]->Move(2);
              found = true;
            }
          }
...
bool Cage::InsideCage(int x, int y){
  ...
  bool found = false;
  while (!found && i < size) {
    if (x == h[i].GetCellRow() && y == h[i].GetCellCol()) {
      found = true;
    }
    else {
      i++;
